 ...
 * 删除设备 - k6 性能测试场景
 * 服务: device
 * 端点: DELETE /api/device/devices/{id}
 * 场景数: 40
 */
...
const ENDPOINT = '/api/device/devices/00000000-0000-0000-0000-000000000001';
const MOCK_TOKEN = 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test';

// 自定义指标
const errorRate = new Rate('errors');
const responseTrend = new Trend('response_time_device_delete');
const requestCounter = new Counter('requests_device_delete');
...
export function smoke_basic() {
  const res = http.delete(BASE_URL + ENDPOINT, mockBody, { headers });
...
export function load_batch_request() {
  const requests = [
    ['DELETE', BASE_URL + ENDPOINT, mockBody, { headers }],
    ['DELETE', BASE_URL + ENDPOINT, mockBody, { headers }],
    ['DELETE', BASE_URL + ENDPOINT, mockBody, { headers }],
  ];
...
export function load_sequential() {
  for (let i = 0; i < 5; i++) {
    http.delete(BASE_URL + ENDPOINT, mockBody, { headers });
    sleep(0.1);
  }
...
export function throughput_rps_50() {
  const requests = [];
  for (let i = 0; i < 10; i++) {
    requests.push(['DELETE', BASE_URL + ENDPOINT, mockBody, { headers }]);
  }
  http.batch(requests);
...
export function reliability_timeout() {
  const res = http.delete(BASE_URL + ENDPOINT, mockBody, { headers, timeout: '10s' });
...
export function reliability_idempotent() {
  const res1 = http.delete(BASE_URL + ENDPOINT, mockBody, { headers });
  const res2 = http.delete(BASE_URL + ENDPOINT, mockBody, { headers });
