 ...
 * 数据摄入管道 - k6 性能测试场景
 * 服务: ingestion
 * 端点: POST /api/ingestion/pipeline
 * 场景数: 35
 */
...
const ENDPOINT = '/api/ingestion/pipeline';
const MOCK_TOKEN = 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test';

const errorRate = new Rate('errors');
const responseTrend = new Trend('response_time_ingestion_pipeline');
const requestCounter = new Counter('requests_ingestion_pipeline');
...
const mockBody = JSON.stringify({
  name: 'k6-test-ingestion_pipeline',
  code: 'K6TEST',
  status: 'active',
  description: 'k6性能测试-数据摄入管道'
});

// ==================== 冒烟测试场景 ====================
export function smoke_basic_ingestion_pipeline() {
...
export function smoke_response_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
...
export function smoke_no_error_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
...
export function smoke_headers_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[S04] Content-Type': r => r.headers['Content-Type'] !== undefined });
  sleep(0.1);
}

export function smoke_body_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
...
// ==================== 负载测试场景 ====================
export function load_10vu_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L01] 10VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_50vu_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L02] 50VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_100vu_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L03] 100VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_1m_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L04] 1分钟持续': r => r.status < 500 });
  sleep(1);
}

export function load_5m_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L05] 5分钟持续': r => r.status < 500 });
  sleep(1);
}

export function load_rampup_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L06] 爬坡正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_rampdown_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[L07] 降载正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_batch_ingestion_pipeline() {
  const requests = [
...
export function load_retry_ingestion_pipeline() {
  let success = false;
...
export function load_sequential_ingestion_pipeline() {
  for (let i = 0; i < 5; i++) {
...
// ==================== 压力测试场景 ====================
export function stress_high_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[ST01] 高负载': r => r.status < 500 });
  sleep(0.1);
}

export function stress_spike_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[ST02] 峰值压力': r => r.status < 500 });
}

export function stress_sustained_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[ST03] 持续压力': r => r.status < 500 });
  sleep(0.2);
}

export function stress_recovery_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  check(res, { '[ST04] 恢复测试': r => r.status < 500 });
  sleep(1);
}

export function stress_200vu_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
...
// ==================== 响应时间测试场景 ====================
export function perf_p50_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p90_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p95_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p99_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, mockBody, { headers });
  responseTrend.add(res.timings.duration);
...
// ==================== 安全测试场景 ====================
export function security_no_auth_ingestion_pipeline() {
...
export function security_invalid_token_ingestion_pipeline() {
...
export function security_wrong_tenant_ingestion_pipeline() {
...
// ==================== 并发安全场景 ====================
export function concurrent_read_ingestion_pipeline() {
...
export function concurrent_mixed_ingestion_pipeline() {
  const requests = [
...
// ==================== 数据边界场景 ====================
export function boundary_empty_body_ingestion_pipeline() {
  const res = http.post(BASE_URL + ENDPOINT, '{}', { headers });
  check(res, { '[BD01] 空Body': r => r.status < 500 });
  sleep(0.3);
}

export function boundary_large_payload_ingestion_pipeline() {
...
export function boundary_special_chars_ingestion_pipeline() {
