...
const ENDPOINT = '/api/analytics/report';
const MOCK_TOKEN = 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test';

const errorRate = new Rate('errors');
const responseTrend = new Trend('response_time_analytics_report');
const requestCounter = new Counter('requests_analytics_report');
...
const mockBody = JSON.stringify({
  name: 'k6-test-analytics_report',
  code: 'K6TEST',
  status: 'active',
  description: 'k6性能测试-分析报告'
});

// ==================== 冒烟测试场景 ====================
export function smoke_basic_analytics_report() {
...
export function smoke_response_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
...
export function smoke_no_error_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
...
export function smoke_headers_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[S04] Content-Type': r => r.headers['Content-Type'] !== undefined });
  sleep(0.1);
}

export function smoke_body_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
...
// ==================== 负载测试场景 ====================
export function load_10vu_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L01] 10VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_50vu_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L02] 50VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_100vu_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L03] 100VU正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_1m_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L04] 1分钟持续': r => r.status < 500 });
  sleep(1);
}

export function load_5m_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L05] 5分钟持续': r => r.status < 500 });
  sleep(1);
}

export function load_rampup_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L06] 爬坡正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_rampdown_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[L07] 降载正常': r => r.status < 500 });
  sleep(0.5);
}

export function load_batch_analytics_report() {
  const requests = [
...
export function load_retry_analytics_report() {
  let success = false;
...
export function load_sequential_analytics_report() {
  for (let i = 0; i < 5; i++) {
...
// ==================== 压力测试场景 ====================
export function stress_high_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[ST01] 高负载': r => r.status < 500 });
  sleep(0.1);
}

export function stress_spike_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[ST02] 峰值压力': r => r.status < 500 });
}

export function stress_sustained_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[ST03] 持续压力': r => r.status < 500 });
  sleep(0.2);
}

export function stress_recovery_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  check(res, { '[ST04] 恢复测试': r => r.status < 500 });
  sleep(1);
}

export function stress_200vu_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
...
// ==================== 响应时间测试场景 ====================
export function perf_p50_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p90_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p95_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  responseTrend.add(res.timings.duration);
...
export function perf_p99_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, null, { headers });
  responseTrend.add(res.timings.duration);
...
// ==================== 安全测试场景 ====================
export function security_no_auth_analytics_report() {
...
export function security_invalid_token_analytics_report() {
...
export function security_wrong_tenant_analytics_report() {
...
// ==================== 并发安全场景 ====================
export function concurrent_read_analytics_report() {
...
export function concurrent_mixed_analytics_report() {
  const requests = [
    ['GET', BASE_URL + ENDPOINT, null, { headers }],
    ['GET', BASE_URL + ENDPOINT, null, { headers }],
    ['GET', BASE_URL + ENDPOINT, null, { headers }],
...
// ==================== 数据边界场景 ====================
export function boundary_empty_body_analytics_report() {
  const res = http.get(BASE_URL + ENDPOINT, '{}', { headers });
  check(res, { '[BD01] 空Body': r => r.status < 500 });
  sleep(0.3);
}

export function boundary_large_payload_analytics_report() {
...
export function boundary_special_chars_analytics_report() {
