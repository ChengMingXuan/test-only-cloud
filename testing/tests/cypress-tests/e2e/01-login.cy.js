...
describe('登录页面 - Mock API 测试', () => {
  ...
  it('[P0] 空表单提交显示验证错误', () => {
    // 清空字段后点登录
    cy.get('input[name=username], input[id*=username], input[id*=userName], input[placeholder*=用户名], input[placeholder*=账号], input[type=text]', { timeout: 8000 }).filter(':visible').first().clear({ force: true });
    cy.get('input[name=password], input[id*=password], input[type=password]', { timeout: 8000 }).filter(':visible').first().clear({ force: true });
  ...
  it('[P0] 有效凭证登录请求成功', () => {
    cy.get('input[name=username], input[id*=username], input[id*=userName], input[placeholder*=用户名], input[placeholder*=账号], input[type=text]', { timeout: 8000 }).filter(':visible').first().clear({ force: true }).type('admin', { force: true });
    cy.get('input[name=password], input[id*=password], input[type=password]', { timeout: 8000 }).filter(':visible').first().clear({ force: true }).type('admin123', { force: true });
    cy.get('button[type=submit], .ant-btn-primary, #login-btn', { timeout: 8000 }).first().click({ force: true });
    // 登录成功后显示主布局
    cy.get('#root, .ant-layout, body', { timeout: 20000 }).should('be.visible');
  });
  ...
  it('[P1] 无效凭证显示服务器错误', () => {
    cy.get('input[name=username], input[id*=username], input[id*=userName], input[placeholder*=用户名], input[placeholder*=账号], input[type=text]', { timeout: 8000 }).filter(':visible').first().clear({ force: true }).type('invalid_user', { force: true });
    cy.get('input[name=password], input[id*=password], input[type=password]', { timeout: 8000 }).filter(':visible').first().clear({ force: true }).type('wrong_pass', { force: true });
    cy.get('button[type=submit], .ant-btn-primary, #login-btn', { timeout: 8000 }).first().click({ force: true });
