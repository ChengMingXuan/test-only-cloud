...
describe('[UI] 区块链-验证', () => {
  ...
  describe('页面加载', () => {
    it('[T001] 页面正常加载 - 根容器存在', () => {
      cy.visitAuth('/blockchain/transactions');
  ...
  describe('权限验证', () => {
    it('[T011] 无 Token 时跳转登录', () => {
      ...
      cy.visit('/blockchain/transactions', { failOnStatusCode: false });
    ...
    it('[T015] 无权限时显示提示', () => {
      cy.intercept('GET', '**/api/blockchain/**', { statusCode: 403, body: { message: '无权限' } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });
  ...
  describe('列表功能', () => {
    ...
    it('[T031] 空数据展示', () => {
      cy.intercept('GET', '**/api/blockchain/**', { body: { success: true, data: { items: [], total: 0 } } });
      cy.visitAuth('/blockchain/transactions');
  ...
  describe('UI 交互', () => {
    ...
    it('[T101] 错误消息提示', () => {
      cy.intercept('POST', '**/api/blockchain/**', { statusCode: 500, body: { message: '服务器错误' } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });
  ...
  describe('异常处理', () => {
    it('[T111] 500 错误处理', () => {
      cy.intercept('GET', '**/api/blockchain/**', { statusCode: 500 });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T112] 404 错误处理', () => {
      cy.intercept('GET', '**/api/blockchain/**', { statusCode: 404 });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T113] 网络超时处理', () => {
      cy.intercept('GET', '**/api/blockchain/**', { delayMs: 5000, body: { success: true, data: [] } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });
    ...
    it('[T115] 403 无权限处理', () => {
      cy.intercept('GET', '**/api/blockchain/**', { statusCode: 403 });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T116] 接口返回异常数据', () => {
      cy.intercept('GET', '**/api/blockchain/**', { body: { success: false, data: null } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });
  ...
  describe('API Mock 验证', () => {
    ...
    it('[T127] Mock 导出接口', () => {
      cy.intercept('GET', '**/api/blockchain/**/export', { body: '' }).as('export');
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T128] Mock 上传接口', () => {
      cy.intercept('POST', '**/api/blockchain/**/upload', { body: { success: true, data: { url: 'mock.png' } } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T129] Mock 批量接口', () => {
      cy.intercept('POST', '**/api/blockchain/**/batch', { body: { success: true } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });

    it('[T130] Mock 统计接口', () => {
      cy.intercept('GET', '**/api/blockchain/**/stats', { body: { success: true, data: { count: 100, total: 1000 } } });
      cy.visitAuth('/blockchain/transactions');
      cy.get('body').should('exist');
    });
  });
