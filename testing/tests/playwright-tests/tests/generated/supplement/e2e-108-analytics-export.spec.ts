...
const PAGE_URL = '/analytics/export';
...
test.describe('[E2E-补充] 数据导出', () => {
  ...
  test.describe('页面访问', () => {
    ...
    test('[A003] 路由地址正确', async ({ page }) => {
      await page.goto(PAGE_URL);
      expect(page.url()).toContain('/analytics/export');
    });
  ...
  test.describe('错误处理', () => {
    ...
    test('[E004] 404页面处理', async ({ page }) => {
      await page.goto('/analytics/export/nonexistent-sub-page');
      await page.waitForTimeout(2000);
      expect(await page.locator('body').textContent()).toBeDefined();
    });
