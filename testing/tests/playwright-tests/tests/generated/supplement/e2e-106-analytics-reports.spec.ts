...
const PAGE_URL = '/analytics/reports';
...
test.describe('[E2E-补充] 分析报表', () => {
  ...
  test.describe('错误处理', () => {
    ...
    test('[E004] 404页面处理', async ({ page }) => {
      await page.goto('/analytics/reports/nonexistent-sub-page');
      await page.waitForTimeout(2000);
      expect(await page.locator('body').textContent()).toBeDefined();
    });
