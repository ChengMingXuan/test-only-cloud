...
const PAGE_URL = '/finance/billing';
...
test.describe('[E2E-补充] 财务计费', () => {
  ...
  test.describe('错误处理', () => {
    ...
    test('[E004] 404页面处理', async ({ page }) => {
      await page.goto('/finance/billing/nonexistent-sub-page');
      await page.waitForTimeout(2000);
      expect(await page.locator('body').textContent()).toBeDefined();
    });
