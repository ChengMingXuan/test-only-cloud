...
const PAGE_URL = '/report/templates';
...
test.describe('[E2E-补充] 报表模板', () => {
  ...
  test.describe('错误处理', () => {
    ...
    test('[E004] 404页面处理', async ({ page }) => {
      await page.goto('/report/templates/nonexistent-sub-page');
      await page.waitForTimeout(2000);
      expect(await page.locator('body').textContent()).toBeDefined();
    });
