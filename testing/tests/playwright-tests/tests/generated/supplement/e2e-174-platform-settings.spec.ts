...
const PAGE_URL = '/platform/settings';
...
test.describe('[E2E-补充] 平台设置', () => {
  ...
  test.describe('错误处理', () => {
    ...
    test('[E004] 404页面处理', async ({ page }) => {
      await page.goto('/platform/settings/nonexistent-sub-page');
      await page.waitForTimeout(2000);
      expect(await page.locator('body').textContent()).toBeDefined();
    });
